...
# 检查是否存在 ./dist/build-YYYYMMDD 文件夹，如果存在则检查其中是否存在 {PROGRAM_NAME}-v{version}-{win64|linux64}.zip 文件
# 如果没有，则检查该目录下是否存在 PROGRAM_NAME ELF/EXE 文件，如果有则打包为 zip 文件
dist_folder = f"./dist/build-{datetime.datetime.now().strftime('%Y%m%d')}-{version}"
if os.path.isdir(dist_folder):
    for arch in ["win64", "linux64"]:
        if arch == 'win64' and os.name != 'nt':
            continue
        elif arch == 'linux64' and os.name != 'posix':
            continue
        
        zip_name = f"{PROGRAM_NAME}-v{version}-{arch}.zip"
        zip_path = os.path.join(dist_folder, zip_name)
        if os.path.exists(zip_path):
            os.remove(zip_path)
        # 查找可能的可执行文件
        if arch == 'win64':
            patterns = [os.path.join(dist_folder, f"{PROGRAM_NAME}.exe")]
        elif arch == 'linux64':
            patterns = [os.path.join(dist_folder, f"{PROGRAM_NAME}")]
        else:
